...
class RetroTester
  attr_reader :api_toucher, :percent_down_to_buy, :percent_up_to_sell, :year_slope_ceiling, :year_slope_floor
  attr_accessor :data_arry, :total_output
  def initialize()
    #@data_arry = GoogleStockScraper.new.results
    @api_toucher = StockApiToucher.new
    @percent_down_to_buy = -3
  ...
  # grab historical data for past 5 years
    ...
    if arry.is_a? Array
      ...
      (arry.count - start).times { |i|   # [251..1256]
       ...
       if percent_change < percent_down_to_buy
